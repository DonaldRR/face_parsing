...
def get_arguments():
    ...
    parser.add_argument("--name", type=str, default='no_edge_v2_deep_only',
        ...
        with tqdm.tqdm(total=total_iters, position=0) as pbar:
            for epoch in range(args.start_epoch, args.epochs):
                ...
                for i_iter, batch in enumerate(trainloader):
                    ...
                    loss = loss_parse * 2 + loss_intra_s * 0 + loss_intra_d * .5+ loss_inter_s * 0 + loss_inter_d * .5 + loss_reg1 * .5 + loss_reg2 * 0
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
